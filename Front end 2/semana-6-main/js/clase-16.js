...
//    Esta funcion retorna una promesa, por eso capturamos su resultado con el then()
    ...
    consultaAsincrona("endpoint").then( respuesta => renderizarElementos(respuesta)) //comentar para llamar a una de las 2 funciones
    consultaAsincrona("endpoint").then( respuesta => renderizarElementosConMap(respuesta))   //comentar para llamar a una de las 2 funciones
})
...
function renderizarElementosConMap(listados){
    // desarrollar la funcion 👇

    const mostrarComentarios = document.querySelector('.comentarios'); 
    mostrarComentarios.innerHTML = ""; 

    listados.map( function(listadoLocal) {
        mostrarComentarios.innerHTML+= `<div class="comentario">         
                <h3>${listadoLocal.name}</h3>
                <h4>${listadoLocal.email}</h4>
                <p>${listadoLocal.body}</p>                        
                </div>`            
    })

    //mostrarComentarios.innerHTML += listadoNuevo.join(" ")
    console.log('con map')
}
